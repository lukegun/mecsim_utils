...
class MECSIM_DataModel:
    ...
    def transform(self):
        ...
        MECSettings[18] = (
            0  # sets the output type to take the below values in  2 for custom #TODO this just turns it off
        )
    ...
    def general_transform(self):
        ...
        Currenttot = np.zeros(
            self.nstoremax
        )  # the plus ones are due to fortran starting at zero
    ...
    def custom_pottime(self, MECSettings, Numberin):
        ...
        if MECSettings[18] == 2:  # load in custom values
            ...
            Ecustom[: Numberin[0]] = np.linspace(
                0.01, 0.8, num=Numberin[0]
            )  # Custom voltage
            Tcustom[: Numberin[0]] = np.linspace(
                0.0, 14, num=Numberin[0]
            )  # Custom time
