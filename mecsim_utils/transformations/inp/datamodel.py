...
class INP_DataModel:
    ...
    def listmapping(self):
        ...
        self.mapping_forward = {"temp": 0, # Temperature (K)
                                ...
                                "geotype": 22 + self.spaces_dic["erev"] ,    # Geometry type (1=planar, 2=spherical, 3=cylinder, 4=RDE)
                                "area": 23 + self.spaces_dic["erev"], # Planar: surface area of electrode (cm^2)
                                "nsph": 24 + self.spaces_dic["erev"], # Number of spheres (can be fractional)
                                "radsph": 25 + self.spaces_dic["erev"],  # Radius of single sphere (cm)
                                "ncycl": 26 + self.spaces_dic["erev"],   # Number of cylinders (can be fractional)
                                ...
                                "nac": 1 + self.spaces_dic['ind_rotation'],   # number of AC sources to add (keep 1 with zero amplitude if want DC only)
                                "AC": 2 + self.spaces_dic['ind_rotation'], # start of the frequency data
                                "nsol": self.spaces_dic['ind_solution'] , # number of species (need n lines below)
                                "conc": 1 + self.spaces_dic['ind_solution'], # storing the sol stuff start
                                "ncap": 1 + self.spaces_dic['ind_solution'] + self.spaces_dic["conc"], # maximum a^n term for a capacitance given by C = a_0 + a_1 V + a_2 V^2 + ... + a_n V^n,
                                "Epzc": 2 + self.spaces_dic['ind_solution'] + self.spaces_dic["conc"], # Epzc (Volts)
                                "cap": 3 + self.spaces_dic['ind_solution'] + self.spaces_dic["conc"], # a_0 term (F/cm^2)
    ...
    def data_finder_inp(self, data):
        ...
        self.spaces_dic = {"ind_rotation":Crot, "ind_solution":Csol, "ind_reac":Ckin,
                    "conc":Nsol, "AC":Nac,"erev":nrev,
